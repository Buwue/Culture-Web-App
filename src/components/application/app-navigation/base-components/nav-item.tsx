...
const styles = sortCx({
  root: "group relative flex max-h-9 w-full cursor-pointer items-center rounded-md bg-[#FBF7F5] outline-focus-ring transition duration-100 ease-linear select-none hover:bg-primary_hover focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2",
...
export const NavItemBase = ({
  ...
  const iconElement = Icon && (
    <Icon
      aria-hidden="true"
      className={cx(
        "transition-inherit-all group-hover/item:text-fg-quaternary_hover mr-2 size-5 shrink-0 text-base text-[#202153]",
  ...
  const labelElement = (
    <span
      className={cx(
        "transition-inherit-all group-hover/item:text-secondary_hover flex-1 text-base font-semibold",
        truncate && "truncate",
